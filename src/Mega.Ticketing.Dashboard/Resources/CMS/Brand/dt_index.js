			...
			columns: [
				{
					...
					template: function (row) {
						if (row.Name) {
							...
							return '<span class="kt-badge kt-badge--danger kt-badge--inline kt-badge--pill">بدون عنوان برند</span>';
						}
					}
				}, {
					field: 'SiteLink',
					...
					template: function (row) {
						if (row.SiteLink) {
							...
							return '<span class="kt-badge kt-badge--danger kt-badge--inline kt-badge--pill">بدون سایت مرجع</span>';
						}
					}
				}, {
					field: 'CountryName',
					title: 'نام کشور تولید کننده',
					template: function (row) {
						if (row.CountryName) {
							return row.CountryName;
						} else {
							return '<span class="kt-badge kt-badge--danger kt-badge--inline kt-badge--pill">بدون کشور</span>';
						}
					}
				}, {
					field: 'IsActive',
					...
					template: function (data, i) {
						return '\
						<a href="javascript:;" class="btn btn-sm btn-clean btn-icon btn-icon-md brand-edit" title="ویرایش" data-id="'+ data.ID + '">\
							<i class="la la-edit"></i>\
						</a>\
						<a href="javascript:;" class="btn btn-sm btn-clean btn-icon btn-icon-md brand-img" title="ثبت عکس" data-id="'+ data.ID + '">\
							<i class="la la-file-image-o"></i>\
						</a>\
						<a href="javascript:;" class="btn btn-sm btn-clean btn-icon btn-icon-md brand-delete" title="حذف" data-id="'+ data.ID + '">\
...
jQuery(document).ready(function () {
	...
	$(document).on('click', '#createBrand', function () {
		MegaYadakModal.inst.Modal('#create', '/CMSbrand/Create', 900, 600, false);
	});
	$(document).on('click', '.brand-edit', function () {
		var id = $(this).attr('data-id');
		MegaYadakModal.inst.Modal('#edit', `/CMSbrand/Edit/${id}`, 900, 600, false);
	});
	...
	$(document).on('click', '.brand-delete', function () {
		...
		Swal.fire({
			...
			text: "این برند بصورت کامل حذف می شود",
			icon: 'warning',
