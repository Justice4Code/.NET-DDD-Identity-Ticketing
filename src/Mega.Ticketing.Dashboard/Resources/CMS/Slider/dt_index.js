			...
			columns: [
				{
					...
					template: function (row) {
						if (row.Link) {
							...
							return '<span class="kt-badge kt-badge--danger kt-badge--inline kt-badge--pill">بدون لینک</span>';
						}
					}
				}, {
					...
					template: function (data, i) {
						return '\
						<a href="javascript:;" class="btn btn-sm btn-clean btn-icon btn-icon-md Slider-edit" title="ویرایش" data-id="'+ data.ID + '">\
							<i class="la la-edit"></i>\
						</a>\
						<a href="javascript:;" class="btn btn-sm btn-clean btn-icon btn-icon-md Slider-img" title="ثبت عکس" data-id="'+ data.ID + '">\
							<i class="la la-file-image-o"></i>\
						</a>\
						<a href="javascript:;" class="btn btn-sm btn-clean btn-icon btn-icon-md Slider-delete" title="حذف" data-id="'+ data.ID + '">\
...
jQuery(document).ready(function () {
	...
	$(document).on('click', '#createSlider', function () {
		MegaYadakModal.inst.Modal('#create', '/CMSSlider/Create', 900, 300, false);
	});
	$(document).on('click', '.Slider-edit', function () {
		var id = $(this).attr('data-id');
		MegaYadakModal.inst.Modal('#edit', `/CMSSlider/Edit/${id}`, 900, 300, false);
	});
	...
	$(document).on('click', '.Slider-delete', function () {
		...
		Swal.fire({
			...
			if (result.value) {
				$.ajax({
					url: "/CMSSlider/AjaxDelete?id=" + id,
					method: "POST",
